...
ll x,y,a,b;
bool possible(ll gift){
	ll start = 0;
	ll end = gift;
	//cout<<"\n--------------------\n";
	while(start<=end){
		ll mid = (start+end)/2;
		ll requiredRed = (mid*a)+((gift-mid)*b);
		ll requiredBlue = (mid*b)+((gift-mid)*a);


		//cout<<start<<sp<<end<<sp<<mid<<sp<<requiredRed<<sp<<requiredBlue<<endl;
		if(x<requiredRed && y<requiredBlue)
			return false;
		if(x<requiredRed){
			end = mid-1;
			continue;
