...
int findU(vector<int> &data,int d,int s){
	int e = data.size()-1;
	int ans = -1;
	while(s<=e){
		int m = (s+e)/2;
...
int findL(vector<int> &data,int d,int s){
	int e = data.size()-1;
	int ans = -1;
	while(s<=e){
		int m = (s+e)/2;
...
void solve(){
	...
	for(int i=0;i<n;i++){
		...
		int i2 = findU(data,upper,i+1);
		int i1 = findL(data,lower,i+1);

		//cout<<data[i]<<sp<<lower<<sp<<upper<<sp<<i1<<sp<<i2<<endl;
		if(i1!=-1 && i2!=-1 && i1<=i2){
			ans+=(i2-i1+1);
		}
	}
