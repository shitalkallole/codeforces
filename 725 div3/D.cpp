...
//to get smallest LPF
int lmt = 1;
vector<int> smallest(lmt);
void LPF(){
	vector<bool> prime(lmt,true);
...
int countPrimeFactors(int V){
	int c = 0; 
	int i=0;
	int n = primeNumbers.size();
	while(i<n){
		if((primeNumbers[i]*primeNumbers[i])<=V){
			if(V%primeNumbers[i]==0){
				c+=1;
				V /= primeNumbers[i];
			}
			else
...
void solve(){
	...
	/*for(int i=0;i<100;i++)
		cout<<i<<sp<<smallest[i]<<endl;*/

	int forA = countPrimeFactors(A);
	int forB = countPrimeFactors(B);
	...
	//cout<<forA<<sp<<forB<<sp<<g<<sp<<m1<<sp<<m2<<endl;
