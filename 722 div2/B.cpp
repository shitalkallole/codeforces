...
void solve(){
	...
	int z=0,neg=0,pos=0;
	int largeNeg=INT_MIN, smallPos=INT_MAX;
	vector<int> data(n);
	for(int i=0;i<n;i++){
		cin>>data[i];
		...
		if(data[i]<0){
			neg+=1;
			largeNeg = max(largeNeg,data[i]);
		}
		else if(data[i]>0){
			pos+=1;
			smallPos = min(smallPos,data[i]);
		}
		else
		{
			z+=1;
	...
	if(neg>=1){
		...
		for(int i=1;i<n;i++){
			if(data[i]<0){
				largeNeg = min(largeNeg,abs(data[i]-data[i-1]));
			}else
			{
				break;
			}
