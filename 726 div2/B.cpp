...
ll dist(pair<ll,ll> F, pair<ll,ll> S){
...
void solve(){
	...
	bool got = false;
	ll sum  = 0;
	pair<ll,ll> person =  {pX,pY};
	vector<pair<ll,ll>> corner = {{1,1},{1,m},{n,1},{n,m}};
	for(int i=0;i<4;i++){
		for(int j=0;j<4;j++){
			ll inter = dist(person,corner[i])+dist(corner[i],corner[j])+dist(corner[j],person);
			if(got){
				if(sum<inter){
					sum = inter;
					a1 = i;
	...
	cout<<corner[a1].first<<sp<<corner[a1].second<<sp<<corner[a2].first<<sp<<corner[a2].second<<endl;
