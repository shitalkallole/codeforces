...
void solve(){
	...
	for(int i=0;i<n;i++){
		if(point[i].second<point[i].first)
			swap(point[i].second,point[i].first);
	}
	...
	for(int i=1;i<n;i++){
		if(point[i].first==point[i].second){
			...
			if(i==1){
				current=point[i].second-point[i].first;
				current+=2;
				current+=(data[i]-1);
				ans=max(ans,current);
			}
			else{
				ll num1 = current - (point[i].second-point[i].first);
				num1+=2+(data[i]-1);

				ll num2 = (point[i].second-point[i].first)+2+(data[i]-1);
				current=max(num1,num2);
				ans=max(ans,current);
			}
		}
	}
