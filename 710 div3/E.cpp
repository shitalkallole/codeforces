...
void solve(){
	...
	vector<int> data(n);
	set<int,greater<int>> forMaximal; 
	
	for(int i=0;i<n;i++){
		cin>>data[i];
		forMaximal.insert(i+1);
	}
	...
	ans[0]=data[0];
	check[ans[0]]=1;
	forMaximal.erase(ans[0]);
	for(int i=1;i<n;i++){
		if(data[i]!=data[i-1]){
			ans[i]=data[i];
			check[ans[i]]=1;
			forMaximal.erase(ans[i]);
		}
	}
	...
	for(int i=0;i<n;i++){
		if(ans[i]==0){
			int value = *forMaximal.lower_bound(last);
			cout<<value<<" ";
			forMaximal.erase(value);
		}
		else{
			last=ans[i];
			cout<<ans[i]<<" ";
