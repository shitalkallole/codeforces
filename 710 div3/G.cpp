...
string filter(string &data,char c){
	string ans;
	int n = data.length();
...
void solve(){
	...
	while(m>0){
		...
		for(int i=0;i<26;i++){
			if(val[i]){
				if(uniqueChar(filter(data,(char)('a'+i)))==m-1){
					push=max(push,(char)('a'+i));
				}
			}
		}
		data=filter(data,push);
		ans+=push;
		val[push-'a']=0;
		m--;
	}
