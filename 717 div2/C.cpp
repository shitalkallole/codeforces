...
ll knap(int start, int weight, int S, int n, vector<vector<ll>> &dp,vector<ll> &w, vector<ll> &r){
	if(start>=n)
		return 0;
	if(dp[start][weight]!=-1)
		return dp[start][weight];

	if(weight+w[start]<=S){
		ll val1 = r[start]+knap(start+1,weight+w[start],S,n,dp,w,r);
		ll val2 = knap(start+1,weight,S,n,dp,w,r);
	
		dp[start][weight] = max(val1,val2);
	}
	else{
		dp[start][weight] = knap(start+1,weight,S,n,dp,w,r);
	}
...
void solve(){
	...
	int total = (n*2000);
	vector<bool> bottomUp(total+1,false);
	bottomUp[0]=1;

	for(int i=0;i<n;i++){
		for(int j=total-data[i];j>=0;j--){
			if(bottomUp[j]){
				bottomUp[j+data[i]]=1;
			}
		}
	}
	if(sum%2 || !bottomUp[sum/2]){
		cout<<"0\n";
		return ;
	}
