...
int main(){
    ...
    while(t--){
    	...
    	vector<pair<ll,int>> present(m);
    	int f;
    	for(int i=0;i<n;i++){
    		cin>>f;
    		present[f-1].second+=1;
    	}
    	...
    	for(int j=m-1;j>=0;j--){
    		if(i<j){
    			int k = i+present[j].second;
    			k-=1;
    			k=min(k,j);
    			for(int l=i;l<=k;l++)
    				ans+=present[l].first;
    			if(k==j){
    				ans=ans+(present[j].first*(present[j].second-(j-i+1)));
    			}
    			i=k+1;
    		}
    		else{
    			ans=ans+(present[j].first*present[j].second);
    		}
    	}
    	cout<<ans<<endl;
